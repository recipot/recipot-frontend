...
import { Recipe } from '@/app/recipe/[id]/types/recipe.types';

// 목 데이터 (recipeRecommend.handlers.ts와 동일한 데이터 사용)
...
export const recipeHandlers = [
  http.get('/v1/recipes/public/:id', ({ params }) => {
    const { id } = params;
  ...
  // 인증이 필요한 레시피 상세 조회 (기존 엔드포인트)
  http.get('/v1/recipes/:id', ({ params }) => {
    const { id } = params;
    const recipeId = Number(id);
