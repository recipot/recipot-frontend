...
export const algorithCheckHandlers = [
  // 못 먹는 음식 item POST
    ...
    return HttpResponse.json(
      {
        ...
        analysis: {
          ...
          seafoodCount: categories.filter((id) => [1, 2, 3, 4, 5].includes(id)).length,
          animalCount: categories.filter((id) => [6, 7, 8, 9, 10].includes(id)).length,
        },
        success: true,
      },
      { status: 200 }
