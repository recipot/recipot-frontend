...
export default function KakaoCallback() {
  ...
  const handleAuthCode = useCallback(
    async (code: string) => {
      try {
        setStatus('카카오 인증을 처리하는 중...');

        // authService를 통해 토큰 받기 (GET 요청)
        const tokenData = await authService.getTokenFromCallback(code);
        ...
        setStatus('카카오 로그인 처리 중 오류가 발생했습니다.');
        setTimeout(() => router.push('/'), 2000);
      }
    },
    [router, setRefreshToken, setToken, setUser]
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // 카카오에서 받은 인증 코드를 백엔드로 전달 (실제 플로우)
      handleAuthCode(code);
    } else {
      setStatus('카카오 인증 정보를 찾을 수 없습니다.');
      setTimeout(() => router.push('/'), 2000);
    }
