import React from 'react';
import { fireEvent, render, screen } from '@testing-library/react';
import { describe, vi } from 'vitest';

import EmotionOptionButton from '../EmotionOptionButton';

describe('EmotionOptionButton', () => {
  test('적절한 label 및 color가 올바르게 렌더링된다', () => {
    render(
      <EmotionOptionButton
        label="별로예요"
        color="red"
        selected
        onClick={() => {}}
      />
    );

    const button = screen.getByRole('button', { name: /별로예요/ });
    expect(button).toBeInTheDocument();
    expect(button).toHaveClass('bg-[#FFE2E2]');
    expect(button).toHaveClass('text-feel-free-text');
  });

  test('감정이 선택된 경우 스타일이 적절하게 변경된다.', () => {
    render(
      <EmotionOptionButton
        label="그저 그래요"
        color="yellow"
        selected
        onClick={() => {}}
      />
    );

    const button = screen.getByRole('button', { name: /그저 그래요/ });
    expect(button).toHaveClass('bg-[#FDFAB0]');
    expect(button).toHaveClass('text-feel-soso-text');
    expect(button).toHaveAttribute('aria-pressed', 'true');
  });

  test('감정이 선택되지 않은 경우 비활성화 스타일이 적용된다.', () => {
    render(
      <EmotionOptionButton
        label="그저 그래요"
        color="yellow"
        selected={false}
        onClick={() => {}}
      />
    );

    const button = screen.getByRole('button', { name: /그저 그래요/ });
    expect(button).toHaveClass('bg-gray-100');
    expect(button).toHaveClass('text-gray-500');
    expect(button).toHaveAttribute('aria-pressed', 'false');
  });

  test('onClick 핸들러가 클릭 시 호출된다', () => {
    const handleClick = vi.fn();

    render(
      <EmotionOptionButton
        label="별로예요"
        color="blue"
        onClick={handleClick}
      />
    );

    const button = screen.getByRole('button', { name: /별로예요/ });
    fireEvent.click(button);

    expect(handleClick).toHaveBeenCalledTimes(1);
  });

  test('적절한 color에 따라 올바른 아이콘을 렌더링한다', () => {
    const { container } = render(
      <EmotionOptionButton label="별로예요" color="blue" onClick={() => {}} />
    );

    // EmotionBadIcon이 렌더링되는지 확인
    const svg = container.querySelector('svg');
    expect(svg).toBeInTheDocument();
  });
});
