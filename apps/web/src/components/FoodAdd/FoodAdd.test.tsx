...
describe('FoodAdd', () => {
  ...
  describe('선택된 재료 관리', () => {
    it('선택된 재료가 없을 때 안내 메시지를 표시한다', () => {
      ...
      expect(
        screen.getByText('재료를 2가지 이상 선택 해주세요!')
      ).toBeInTheDocument();
    });

    it('선택된 재료가 있을 때 선택된 재료 목록을 표시한다', () => {
      mockUseSelectedFoodsStore.mockReturnValue({
        addFood: vi.fn(),
        clearAllFoods: vi.fn(),
      ...
      expect(screen.getByText('당근')).toBeInTheDocument();
      expect(
        screen.queryByText('재료를 2가지 이상 선택 해주세요!')
      ).not.toBeInTheDocument();
    });

    it('전체 삭제 버튼을 클릭하면 모든 선택이 해제된다', () => {
      const mockClearAllFoods = vi.fn();
      mockUseSelectedFoodsStore.mockReturnValue({
        addFood: vi.fn(),
        clearAllFoods: mockClearAllFoods,
        getSelectedCount: vi.fn().mockReturnValue(2),
        getSelectedFoods: vi.fn(),
        isSelected: vi
          .fn()
      ...
      expect(mockClearAllFoods).toHaveBeenCalled();
    });

    it('선택된 재료의 X 버튼을 클릭하면 해당 재료가 제거된다', () => {
      const mockToggleFood = vi.fn();
      mockUseSelectedFoodsStore.mockReturnValue({
        addFood: vi.fn(),
        clearAllFoods: vi.fn(),
      ...
      // X 아이콘을 직접 클릭 (SVG는 querySelector로 찾기)
      const firstSelectedFoodSpan = screen.getByText('감자').closest('span');
      const xIcon = firstSelectedFoodSpan?.querySelector('svg');

      if (xIcon) {
        fireEvent.click(xIcon);
        expect(mockToggleFood).toHaveBeenCalled();
      }
    });
  });
